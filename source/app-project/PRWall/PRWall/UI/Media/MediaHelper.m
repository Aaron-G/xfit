...
+ (void) displayImageFullScreen:(Media*) image {
  ...
  imageDisplayViewController.image = [UIImage imageWithContentsOfFile:image.path];
...
+ (void) moveMediaAtIndexPath:(NSIndexPath *)fromIndexPath toIndexPath:(NSIndexPath *)toIndexPath inVideos:(NSArray*) videos inVideosSection:(NSInteger) videosSection orInImages:(NSArray*) images inImagesSection:(NSInteger) imagesSection {
  ...
  if(toIndexPath.section == imagesSection || toIndexPath.section == videosSection) {
    
    if(toIndexPath.item == fromIndexPath.item) {
      return;
    }
    
    NSArray* mediaObjectsToUpdate = nil;
    
    if(toIndexPath.section == imagesSection) {
      mediaObjectsToUpdate = images;
    } else if(toIndexPath.section == videosSection) {
      mediaObjectsToUpdate = videos;
    }
    
    NSInteger indexOfMediaObjectToMoveBeforeMove = fromIndexPath.item-1;
    NSInteger indexOfMediaObjectToMoveAfterMove = toIndexPath.item-1;
    
    //Update index of moved object
    Media* mediaObject = [mediaObjectsToUpdate objectAtIndex: (indexOfMediaObjectToMoveBeforeMove)];
    mediaObject.index = [NSNumber numberWithInt: indexOfMediaObjectToMoveAfterMove];
    
    NSInteger startIndexOfObjectsToUpdate =  0;
    NSInteger endIndexOfObjectsToUpdate = 0;
    NSInteger indexIncrement = 0;
    
    //Moving down
    if(fromIndexPath.item < toIndexPath.item) {
      startIndexOfObjectsToUpdate = indexOfMediaObjectToMoveBeforeMove + 1;
      endIndexOfObjectsToUpdate = indexOfMediaObjectToMoveAfterMove;
      indexIncrement = -1;
    }
    
    //Moving up
    else {
      startIndexOfObjectsToUpdate = indexOfMediaObjectToMoveAfterMove;
      endIndexOfObjectsToUpdate = indexOfMediaObjectToMoveBeforeMove - 1;
      indexIncrement = +1;
    }
    
    //Adjust index of impacted objects
    for (Media* media in mediaObjectsToUpdate) {
      NSInteger mediaIndex = media.index.integerValue;
      
      if([media isEqual:mediaObject]) {
        continue;
      }
      
      if((startIndexOfObjectsToUpdate <= mediaIndex) && (mediaIndex <= endIndexOfObjectsToUpdate)) {
        media.index = [NSNumber numberWithInt: ([media.index integerValue] + indexIncrement)];
      }
    }
  }
}
