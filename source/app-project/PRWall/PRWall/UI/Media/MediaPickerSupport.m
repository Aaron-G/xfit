...
@property NSIndexPath* indexPathToPick;
...
- (void)startPickingMediaAtIndexPath:(NSIndexPath *)indexPath {
  ...
  self.indexPathToPick = indexPath;
  
  if(indexPath.section == self.delegate.imagesSection) {
  ...
  } else if(indexPath.section == self.delegate.videosSection) {
...
- (void) addImage:(UIImage*) image orVideo:(NSURL*) video inSection:(NSInteger) section {
  ...
  if(image) {
    [self.delegate pickedImage: mediaURL.path atIndexPath:self.indexPathToPick];
  } else if (video) {
    [self.delegate pickedVideo: mediaURL.path atIndexPath:self.indexPathToPick];
  }
